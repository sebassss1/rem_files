...
namespace Basis
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            BNL.Log($"[PSP-RA4] Integration Strategy Configured:");
            BNL.Log($" - Auth Service: {config.ExternalAuthServiceUrl ?? "N/A"}");
            BNL.Log($" - Database Service: {config.ExternalDatabaseServiceUrl ?? "N/A"}");
            BNL.Log($" - Analytics Service: {config.ExternalAnalyticsServiceUrl ?? "N/A"}");
            ...
            AppDomain.CurrentDomain.ProcessExit += async (sender, eventArgs) =>
            {
                ...
                BasisPersistentDatabase.ShutdownStatic();
